...
public class Config {
	...
	static {
		...
		ROTATION_SPEED = CLIENT_BUILDER.comment("Rotation speed of the magic block").defineInRange("rotationSpeed",
				100.0, 0.0, 1000000.0);
		CLIENT_BUILDER.pop();
	...
	private static void setupFirstBlockConfig(ForgeConfigSpec.Builder SERVER_BUILDER,
		...
		FIRSTBLOCK_GENERATE = SERVER_BUILDER.comment("Power generation per diamond").defineInRange("generate", 1000, 0,
				Integer.MAX_VALUE);
		FIRSTBLOCK_SEND = SERVER_BUILDER.comment("Power generation to send per tick").defineInRange("send", 100, 0,
				Integer.MAX_VALUE);
		FIRSTBLOCK_TICKS = SERVER_BUILDER.comment("Ticks per diamond").defineInRange("ticks", 20, 0, Integer.MAX_VALUE);
